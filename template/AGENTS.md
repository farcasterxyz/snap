...
- `ctx.action.type === "post"` — user interaction (POST request). Includes `inputs` (Record of input name → value), `fid` (user's Farcaster ID), and `timestamp`. Use different `submit` target URLs (for example query parameters) to distinguish multiple buttons.
...
```bash
PAYLOAD=$(echo -n "{\"fid\":1,\"inputs\":{},\"timestamp\":$(date +%s)}" \
  | base64 | tr '+/' '-_' | tr -d '=')
