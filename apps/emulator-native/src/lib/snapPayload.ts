 ...
 * Duplicated here so the native emulator stays self-contained (plan MVP).
 */
...
/**
 * Ensure `page` matches SPEC: `page.elements` is `{ type: "stack", children: [...] }`.
 */
...
/** Prefer inner `{ version, page }` for Zod validation when the server wraps `snap`. */
