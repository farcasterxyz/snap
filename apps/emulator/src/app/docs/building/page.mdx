...
curl -sS -o .claude/commands/create-farcaster-snap.md \
  https://snap.farcaster.xyz/skill
```
...
Use the [Emulator](/emulator) to test your snap locally. Enter your snap's URL (e.g. `http://localhost:3003`) and interact with it.

For local development, set `SKIP_JFS_VERIFICATION=1` on your snap server to skip JFS signature verification.
