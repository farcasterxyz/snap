...
| `variant` | string | No | `"default"` | Visual style (see below) |
| `icon` | IconName | No | | Leading icon. See [icon set](/elements#icon) |

### Variants

| Variant | Description |
|---------|-------------|
| `default` | Solid accent background, white text — use for the primary CTA |
...
"btn-no": {
  "type": "button",
  "props": { "label": "No", "variant": "outline" },
...
See [Actions](/actions) for parameters and examples for each type.
