...
| `client_action` | Yes* | Structured client action object for `client` buttons. See [Actions](/actions) |
...
Triggers a Farcaster client action. Instead of a `target` URL, `client` buttons use a structured `client_action` object. See the full list of supported client actions on the [Actions](/actions) page.
