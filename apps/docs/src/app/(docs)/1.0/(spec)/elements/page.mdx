...
The only component that fires actions — bind them via `on.press`. Default variant is
`"secondary"` (bordered); use `"primary"` (filled) for the main CTA, typically one per
page. See [Actions](/snap/actions) for the full list of action types.
...
root. Use horizontal stacks for button rows, badge groups, or side-by-side cards — peers
stay on one row and share width. `justify: "between"` is useful for navigation bars.
...
Use `select` to enable tap-to-select; taps write to POST inputs under `name`.
