...
The POST payload shape has changed. v2 adds `audience` and removes `button_index`.
...
Every v2 POST payload includes **`audience`** — the origin of the snap server the
request is intended for.

If you use `@farcaster/snap-hono`, it is validated automatically by `parseRequest`. No
code changes needed on your end — just upgrade the package.
...
Farcaster clients **must** be updated to send `audience` in every POST payload. v2 snap
servers will reject requests that omit this field. See the
...
validates audience, and the response validator enforces structural constraints.
...
- [ ] Ensure your Farcaster client sends `audience` in POST payloads
      ([details](/snap/client-upgrade))
