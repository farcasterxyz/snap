...
```bash
install the farcaster-snap skill from https://docs.farcaster.xyz/snap/2.0/SKILL.md
...
registerSnapHandler(app, async (ctx) => {
  ...
  // Handle POST interactions (v2: nonce/audience are verified automatically)
  const { fid, inputs } = ctx.action;
  // v2 uses distinct submit target URLs instead of button_index
