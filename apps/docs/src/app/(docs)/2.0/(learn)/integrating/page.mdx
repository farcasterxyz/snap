...
If you're building a Farcaster client that displays snaps, keep these display guidelines
in mind:

- **Width**: Snaps are designed for a fixed width of ~480px. Render them in a
  fixed-width container.
- **Height**: Clip snap content at around **500px**. Content taller than this should be
  hidden or require user interaction to reveal. The spec does not mandate a specific
  height — 500px is the conventional limit used by existing clients.
- **No client-side code execution**: Snaps are pure JSON rendered by the client. Never
  execute scripts or inject user-provided HTML.

The `@farcaster/snap` package provides `SnapCard` and `SnapView` React components for
rendering snaps. `SnapCard` accepts a `showOverflowWarning` prop — when `false` (default)
it clips at 500px; when `true` it extends to 700px with a visual overflow indicator.
