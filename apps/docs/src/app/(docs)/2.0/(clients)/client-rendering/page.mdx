...
| `view_channel`     | `({ channelKey: string })`                                                                | Navigate to a Farcaster channel by channel key.                                                                                                      |
...
| `send_transaction` | `({ chainId, to, data?, value?, gas?, gasPrice?, maxFeePerGas?, maxPriorityFeePerGas? })` | Request an EVM transaction through the host wallet. Return a promise so the presenter can expose pending state until the wallet flow settles.        |
...
For `send_transaction`, hosts that support feedback should POST a signed
`transaction_result` callback to the current snap URL after the wallet flow settles. The
server can read it with `parseRequest` as `ctx.action.type === "transaction_result"` and
inspect `ctx.action.transaction.result.transactionHash` on success.
...
function SnapRenderer({
  ...
  const handlers: SnapActionHandlers = {
    ...
    view_channel: ({ channelKey }) => navigateToChannel(channelKey),
    ...
    send_transaction: (params) => sendTransaction(params),
  };
