...
If the server supports snaps, it returns JSON with `version`, `theme`, and `ui`
fields. If not, it returns its normal HTML response.

## SnapCard Component

`SnapCard` is the primary component for rendering a snap. It handles version
detection, validation, and rendering automatically.
...
| `open_url` | `(target: string)` | Open a URL in the system browser or in-app browser. |
...
The `submit` handler is the most important — it's how snaps navigate between pages.
All input values from `input`, `slider`, `switch`, and `toggle_group` elements are
...
4. `SnapCard` receives the error via the `actionError` prop and renders it below
   the snap content, outside the 500px clipped area so it's always visible
