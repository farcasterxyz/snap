...
the renderer shows one child at a time and can provide built-in previous/next controls
and page indicators. The current page index is json-render local UI state and is never included
in POST `inputs`.
...
| `controlsPosition` | string | No     | `"bottom"` | Position for the built-in pagination bar: `"top"` or `"bottom"` |
| `transition`     | string  | No       | `"slide"` | Page-change animation: `"slide"` `"fade"` `"scale"` `"none"` |

Set both `showControls: false` and `showIndicators: false` to hide the built-in
pagination bar. Set `controlsPosition: "top"` to render the bar above page content
when you want the controls and dots to stay fixed while page content changes height.
Use `transition` to choose the local page-change animation, or `"none"` to disable it.
Buttons or tappable `cell_grid` cells anywhere in the same snap can bind local actions:

| Action               | Params                   | Behavior                                  |
| -------------------- | ------------------------ | ----------------------------------------- |
| `paginator_next`     | `{}`                     | Move to the next page locally             |
| `paginator_prev`     | `{}`                     | Move to the previous page locally         |
| `paginator_go_to`    | `{ "page": 0 }`          | Move to a zero-based page index locally   |

These actions affect the snap's rendered paginator and never submit POST input.
Only one paginator is supported per snap in this release.
...
`size: "sm"` for captions, timestamps, and secondary info. Text does not clamp by
default; set `maxLines` when the content should use a bounded number of visible lines.
...
| `maxLines` | number | No      | No clamp   | Visible lines before clamping (1–6) |
...
| `maxWidth`  | string   | No       | `"lg"`       | Maximum grid width token: `"sm"` `"md"` `"lg"`. `"lg"` is full-width/default; constrained grids center themselves |
