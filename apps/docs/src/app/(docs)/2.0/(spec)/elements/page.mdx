...
The go-to component for structured content rows: leaderboards, settings, key-value info. Has a title, optional description, and an actions slot on the right side. Put badges or buttons in `children` for the action slot. Items are not interactive — don't use `chevron-right` as a disclosure affordance.
...
Rendered in the **actions slot** (right side). Use for badges or buttons — the item itself is not clickable, so avoid `chevron-right` or any affordance that implies navigation on the row.
