...
page. ([cell_grid](#cell_grid) also fires actions, via `on.tap` per cell.) See
...
Use `select` to enable tap interaction; taps write to POST inputs under `name` and fire
the `on.tap` event, so you can bind a `submit` action and respond to each tap without
...
| Event | Fires when                                                                 |
| ----- | -------------------------------------------------------------------------- |
| `tap` | A cell is tapped (only when `select` is `"single"` or `"multiple"`). The latest selection is already written to `inputs[name]` before the action runs, so a bound `submit` POST includes the tapped cell. |
...
`select: "single"` — every tap submits the just-tapped cell:
...
```json
{
  ...
  "on": {
    "tap": { "action": "submit", "params": { "target": "https://my-snap.com/" } }
  }
}
```

`select: "multiple"` — taps update the selection without posting; pair with a `button`
that submits the accumulated selection:
