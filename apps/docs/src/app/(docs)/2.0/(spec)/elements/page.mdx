...
Column count is taken from `columns` when set, or inferred from button-row children. An explicit `gap` always wins.
...
1. **Press to act** — leave `select` at `"off"` and bind `on.press`. Each press writes
   `"row,col"` to `inputs[name]` and fires the bound action (e.g. `submit`). No selection
   ring; cells behave like a grid of buttons.
