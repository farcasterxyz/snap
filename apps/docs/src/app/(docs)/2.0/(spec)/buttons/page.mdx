...
| `disabled` | boolean \| state binding | No       | `false`       | Disable the button                                |
...
Place buttons inside a `stack` component. In the React and React Native components
exported by `@farcaster/snap`, when every child in a stack is a button, the component
chooses the orientation from the button labels. This is component behavior, not a snap
protocol requirement, so other snap clients may render differently.

Horizontal rows use total visible characters across all button labels:

| Controls | Max total chars |
| -------- | --------------- |
| 2        | 20              |
| 3        | 15              |
| 4        | 11              |
...
In these components, button sets over those totals render vertically.
...
| client actions  | `view_cast`, `view_profile`, `view_channel`, `compose_cast`, `send_token`, etc. |

See [Actions](/snap/actions) for parameters and examples for each type.

## Pending Actions

Async client actions such as `send_transaction` can expose pending state through the
React and React Native presenters. Add `params.activityKey` to the action, then bind the
button's `disabled` prop to `/actions/<activityKey>/pending`.
