...
For `submit`, `params.target` MUST resolve to the **same origin and pathname** as the snap document URL (query string may differ). Omitting `params.target` is allowed and typically defaults to the snap document URL.
...
Each `submit` button POSTs to the snap document URL (same origin + pathname). **Distinguish intents with `inputs`**, not with different paths: give fields distinct names or values (for example a hidden `input` with `name: "_action"` whose default differs per row, or separate named fields the user edits). On the server, read `action.inputs._action` (or your chosen field) to dispatch.

Example: two buttons, same `params.target`, different intents via inputs on the page (conceptual — your layout may use a hidden field, toggles, or other components):
