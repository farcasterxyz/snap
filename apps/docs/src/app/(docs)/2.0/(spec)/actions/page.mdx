...
| [view_channel](#view_channel)         | Navigate to a channel             |
...
| [send_transaction](#send_transaction) | Request an EVM wallet transaction |
...
Navigate to a Farcaster channel.

| Param        | Type   | Required | Description                          |
| ------------ | ------ | -------- | ------------------------------------ |
| `channelKey` | string | Yes      | Channel key, without a leading slash |

```json
{
  "type": "button",
  "props": { "label": "View Channel" },
  "on": {
    "press": {
      "action": "view_channel",
...
Request a single EVM transaction through the host wallet using `eth_sendTransaction`.
This action does not submit directly from the snap server and does not call a local
`submit` handler. The host opens the wallet flow, waits for it to settle, and may POST a
signed `transaction_result` callback back to the current snap URL.

Use `activityKey` when you want the React or React Native presenter to expose stable
pending state while the wallet flow is in progress. During the async handler, presenters
write:

- `/actions/<activityKey>/name`
- `/actions/<activityKey>/pending`

Buttons can bind `props.disabled` to that pending path so the CTA stays disabled while
the transaction is in flight.

| Param                  | Type   | Required | Description                                     |
| ---------------------- | ------ | -------- | ----------------------------------------------- |
| `chainId`              | string | Yes      | EVM chain ID as a string                        |
| `to`                   | string | Yes      | Recipient or contract address                   |
| `data`                 | string | No       | Hex calldata                                    |
| `value`                | string | No       | Hex value                                       |
| `gas`                  | string | No       | Hex gas limit                                   |
| `gasPrice`             | string | No       | Legacy hex gas price                            |
| `maxFeePerGas`         | string | No       | EIP-1559 max fee per gas                        |
| `maxPriorityFeePerGas` | string | No       | EIP-1559 max priority fee per gas               |
| `activityKey`          | string | No       | Stable render-state key for pending UI bindings |
...
Hosts that support transaction feedback POST a signed callback payload to the snap URL
after the wallet request succeeds or fails. `parseRequest` returns this as
`ctx.action.type === "transaction_result"`.

The callback includes the wallet request under `ctx.action.transaction.request` and the
settlement result under `ctx.action.transaction.result`:

```ts
if (ctx.action.type === "transaction_result") {
  const { request, result } = ctx.action.transaction;
...
Success results include `{ success: true, transactionHash }`. Failure results include
`{ success: false, reason, message?, code?, transactionHash? }`, where `reason` is
`"rejected_by_user"`, `"failed"`, or `"unknown"`.
