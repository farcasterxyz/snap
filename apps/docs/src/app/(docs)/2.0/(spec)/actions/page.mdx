...
> **Pass URLs via `embeds`, never inside `text`.** Clients detect URL embeds
> from the `embeds` array. A URL baked into the `text` string may not be
> recognized as an embed until the user edits the composer, which breaks the
> share-flow UX. Always put the snap URL in `embeds`.
