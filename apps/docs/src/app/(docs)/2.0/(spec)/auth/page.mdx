...
on GET is optional and best-effort: servers SHOULD fall back to an anonymous first load
...
Official clients MAY attach a JFS compact string to snap document GET requests using the
`X-Snap-Payload` request header. The value uses the same dot-separated form as POST
bodies:
...
The decoded JSON payload matches the POST shape **without** `inputs` or deprecated
top-level `fid`: `timestamp`, `audience`, `user`, and `surface`. From official clients,
`surface` on GET is typically `{ "type": "standalone" }`.

- GET authentication is **optional**. Snaps that do not send `X-Snap-Payload` behave as
  today (anonymous `{ type: "get" }`).
- Servers MUST reject a present but malformed, expired, or invalid `X-Snap-Payload` with
  a 4xx response (anonymous fallback is only for the **absent** case). Failing fast
  surfaces client bugs and forgery attempts instead of masking them as anonymous.
...
Servers MUST NOT depend on receiving a viewer FID on any GET — **including from users
who have previously POSTed (authenticated) to this snap before.** Each GET is an
independent request, and `X-Snap-Payload` may be absent for many honest reasons:
...
Design every snap so the **anonymous first load is fully usable**. Treat `action.user`
on GET as a strict enhancement: when present, you may personalize; when absent, render a
working signed-out experience. POST is the only place a verified FID is guaranteed —
gate identity-critical flows there.
