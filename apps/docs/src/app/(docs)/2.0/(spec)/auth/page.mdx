...
- Servers MUST reject a present but malformed, expired, or invalid `X-Snap-Payload`
  with a 4xx response (anonymous fallback is only for the **absent** case). Failing
  fast surfaces client bugs and forgery attempts instead of masking them as anonymous.
