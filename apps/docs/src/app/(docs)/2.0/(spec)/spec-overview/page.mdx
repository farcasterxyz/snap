...
A **snap** is identified by **origin + pathname** only. Query string and fragment are not part of snap identity: two URLs with the same origin and pathname but different queries are the same snap with different parameters.

All `submit` actions within a snap MUST POST to the **same pathname** as the snap document the client originally fetched (same origin + pathname). The button’s `params.target`, when present, MUST resolve to that origin and pathname (query strings may differ). Implementations MAY omit `params.target` and default to the snap document URL — recommended. Use **input values** to distinguish button intents (for example a hidden input `name: "_action"` whose value identifies the button).
...
2. The client GETs that URL, signaling snap support. The server responds with a JSON
   SnapResponse (GET MAY include optional viewer identity via `X-Snap-Payload`; see [Authentication](/snap/auth))
...
The POST is sent to the URL in the button’s `submit` action (`params.target`), which MUST match the snap document’s origin and pathname. Use **inputs** (for example `_action` or per-field values) to tell which button or intent ran — not different paths.
