...
| `client_action` | Yes* | Structured client action object for `client` buttons. See [Actions](/docs/actions) |
...
Triggers a Farcaster client action. Instead of a `target` URL, `client` buttons use a structured `client_action` object. See the full list of supported client actions on the [Actions](/docs/actions) page.
